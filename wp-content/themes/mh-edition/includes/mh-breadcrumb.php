...
if (!function_exists('mh_edition_breadcrumb')) {
	function mh_edition_breadcrumb() {
		if (!is_home() && !is_front_page()) {
			global $post;
			$mh_edition_options = mh_edition_theme_options();
			if ($mh_edition_options['breadcrumbs'] == 'enable') {
				...
				$close_span = '</span>';
				echo '<nav class="mh-breadcrumb">' . $before_link . '<a href="' . esc_url(home_url()) . '" itemprop="url">' . $before_title . __('Home', 'mh-edition') . $close_span . '</a>' . $close_span . $delimiter;
					if (is_single() && get_post_type() == 'post' && !is_attachment()) {
						...
						echo __('Media', 'mh-edition') . $delimiter;
						echo get_the_title();
						...
						echo __('Authors', 'mh-edition') . $delimiter . esc_attr($user_info->display_name);
					} elseif (is_404()) {
						echo __('Page not found (404)', 'mh-edition');
					} elseif (is_search()) {
						echo __('Search', 'mh-edition') . $delimiter . get_search_query();
					} elseif (is_date()) {
						...
						if (is_day()) {
							echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mh-edition') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter;
							echo $before_link . '<a href="' . $url_month . '" title="' . __('Monthly Archives', 'mh-edition') . '" itemprop="url">' . $before_title . $arc_month . $close_span . '</a>' . $close_span . $delimiter . $arc_day . ' (' . $arc_day_full . ')';
						} elseif (is_month()) {
							echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mh-edition') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $arc_month;
						} elseif (is_year()) {
							echo $arc_year;
						}
...
add_action('mh_before_post_content', 'mh_edition_breadcrumb');
add_action('mh_before_page_content', 'mh_edition_breadcrumb');
