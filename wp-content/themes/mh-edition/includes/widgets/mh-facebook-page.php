...
class mh_edition_facebook_page extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_edition_facebook_page', esc_html_x('MH Facebook Page', 'widget name', 'mh-edition'),
			array(
				'classname' => 'mh_edition_facebook_page',
				'description' => esc_html__('MH Facebook Page widget to display your Facebook page on your website.', 'mh-edition')
			)
		);
	}
    function widget($args, $instance) {
    	$defaults = array('title' => __('Follow on Facebook', 'mh-edition'), 'fb_url' => 'https://www.facebook.com/MHthemes', 'width' => 300, 'height' => 500, 'cover' => 0, 'faces' => 1, 'posts' => 0);
        ...
        echo $args['before_widget'];
			...
			if (!empty($instance['fb_url'])) {
				echo '<div class="mh-edition-facebook-page-widget">' . "\n";
    ...
    function form($instance) {
    	$defaults = array('title' => esc_html__('Follow on Facebook', 'mh-edition'), 'fb_url' => 'https://www.facebook.com/MHthemes', 'width' => 300, 'height' => 500, 'cover' => 0, 'faces' => 1, 'posts' => 0);
        $instance = wp_parse_args($instance, $defaults); ?>
		<p>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('fb_url')); ?>"><?php esc_html_e('Facebook Page URL:', 'mh-edition'); ?></label>
       	...
       	<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php esc_html_e('Width (min. 280px):', 'mh-edition'); ?></label>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('height')); ?>"><?php esc_html_e('Height (min. 130px):', 'mh-edition'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('cover')); ?>"><?php esc_html_e('Hide cover photo', 'mh-edition'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('faces')); ?>"><?php esc_html_e('Show profile photos', 'mh-edition'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('posts')); ?>"><?php esc_html_e('Show posts from FB page', 'mh-edition'); ?></label>
		</p><?php
    }
}
