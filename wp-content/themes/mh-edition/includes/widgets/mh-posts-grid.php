...
class mh_edition_posts_grid extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_edition_posts_grid', esc_html_x('MH Posts Grid', 'widget name', 'mh-edition'),
			array(
				'classname' => 'mh_edition_posts_grid',
				'description' => esc_html__('MH Posts Grid widget to display a grid of posts including thumbnail images.', 'mh-edition'),
        ...
        echo $args['before_widget'];
			if ($widget_posts->have_posts()) :
				...
				while ($widget_posts->have_posts()) : $widget_posts->the_post();
					if ($counter == 1) {
						echo '<div class="mh-row clearfix mh-posts-grid-widget">' . "\n";
					}
					if ($counter >= 1) {
					...
					if ($counter % 3 == 0 && $counter != $instance['postcount']) {
						echo '</div>' . "\n";
						echo '<div class="mh-row clearfix mh-posts-grid-widget mh-posts-grid-widget-more">' . "\n";
					}
					$counter++;
				endwhile;
				echo '</div>' . "\n";
