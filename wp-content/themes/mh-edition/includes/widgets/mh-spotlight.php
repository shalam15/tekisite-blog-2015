...
class mh_edition_spotlight extends WP_Widget {
    function __construct() {
		parent::__construct(
			'mh_edition_spotlight', esc_html_x('MH Spotlight', 'widget name', 'mh-edition'),
			array(
				'classname' => 'mh_edition_spotlight',
				'description' => esc_html__('MH Spotlight / Featured widget for use on homepage template.', 'mh-edition'),
    ...
    function widget($args, $instance) {
        ...
        echo $args['before_widget']; ?>
			<article class="mh-spotlight-widget"><?php
				while ($spotlight_loop->have_posts()) : $spotlight_loop->the_post();
					if (!empty($instance['title'])) {
						echo '<div class="mh-spotlight-caption">' . esc_html(apply_filters('widget_title', $instance['title'])) . '</div>';
					} ?>
					<div class="mh-spotlight-thumb">
						<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
							if (has_post_thumbnail()) {
								...
								if ($instance['image_size'] == 'normal') {
									...
									echo '<img class="mh-image-placeholder" src="' . get_template_directory_uri() . '/images/placeholder-slider.png' . '" alt="No Picture" />';
								}
							} ?>
						</a>
					</div>
					<div class="mh-spotlight-content">
						...
						<?php if ($instance['meta'] == 0) { ?>
							<p class="mh-meta mh-spotlight-meta">
								<?php $category = get_the_category(); printf(_x('by %1$s in %2$s', 'post meta', 'mh-edition'), get_the_author(), esc_attr($category[0]->cat_name)); ?>
								<?php if ($mh_edition_options['post_meta_comments'] == 'enable') { ?>
									<span class="mh-spotlight-comments">
										<i class="fa fa-comment-o"></i><?php mh_edition_comment_count(); ?>
									</span>
								<?php } ?>
							</p>
						<?php } ?>
    ...
    function form($instance) {
    	$defaults = array('title' => esc_html__('In the Spotlight', 'mh-edition'), 'category' => 0, 'cats' => '', 'tags' => '', 'offset' => 0, 'order' => 'date', 'image_size' => 'normal', 'excerpt_length' => 35, 'excerpt' => 0, 'meta' => 0);
        $instance = wp_parse_args($instance, $defaults); ?>
        <p>
        ...
        <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('image_size')); ?>"><?php esc_html_e('Image size:', 'mh-edition'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('image_size')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('image_size')); ?>">
				<option value="normal" <?php selected('normal', $instance['image_size']); ?>><?php esc_html_e('Normal', 'mh-edition'); ?></option>
				<option value="large" <?php selected('large', $instance['image_size']); ?>><?php esc_html_e('Large', 'mh-edition'); ?></option>
			</select>
        </p>
        <p>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('meta')); ?>"><?php esc_html_e('Disable Post Meta', 'mh-edition'); ?></label>
		</p><?php
    }
}
