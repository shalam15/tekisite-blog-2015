...
$mh_google_webfonts_subsets = array(
	'arabic' => esc_html__('Arabic', 'mh-cicero'),
	'cyrillic' => esc_html__('Cyrillic', 'mh-cicero'),
	'cyrillic_ext' => esc_html__('Cyrillic Extended', 'mh-cicero'),
	'greek' => esc_html__('Greek', 'mh-cicero'),
	'greek_ext' => esc_html__('Greek Extended', 'mh-cicero'),
	'hebrew' => esc_html__('Hebrew', 'mh-cicero'),
	'latin' => esc_html__('Latin', 'mh-cicero'),
	'latin_ext' => esc_html__('Latin Extended', 'mh-cicero'),
	'vietnamese' => esc_html__('Vietnamese', 'mh-cicero')
);
...
function mh_cicero_typography_options($wp_customize) {
	global $mh_google_webfonts_subsets, $mh_google_webfonts_list;

	/***** Add Section *****/

	$wp_customize->add_section('mh_typo', array('title' => esc_html__('Typography', 'mh-cicero'), 'priority' => 3, 'panel' => 'mh_theme_options'));

    /***** Add Settings *****/

    $wp_customize->add_setting('mh_cicero_options[font_size]', array('default' => 16, 'type' => 'option', 'sanitize_callback' => 'mh_cicero_sanitize_integer'));
	$wp_customize->add_setting('mh_cicero_options[google_webfonts]', array('default' => 'enable', 'type' => 'option', 'sanitize_callback' => 'mh_cicero_sanitize_select'));
	$wp_customize->add_setting('mh_cicero_options[google_webfonts_subsets]', array('default' => 'latin', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts_subsets'));
	$wp_customize->add_setting('mh_cicero_options[font_heading]', array('default' => 'open_sans', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts'));
	$wp_customize->add_setting('mh_cicero_options[font_body]', array('default' => 'open_sans', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts'));
	$wp_customize->add_setting('mh_cicero_options[font_styles]', array('default' => '300,400,400italic,600,700', 'type' => 'option', 'sanitize_callback' => 'mh_cicero_sanitize_text'));

    /***** Add Controls *****/

    $wp_customize->add_control('font_size', array('label' => esc_html__('Change default Font Size (px)', 'mh-cicero'), 'section' => 'mh_typo', 'settings' => 'mh_cicero_options[font_size]', 'priority' => 1, 'type' => 'text'));
    $wp_customize->add_control('google_webfonts', array('label' => esc_html__('Google Webfonts', 'mh-cicero'), 'section' => 'mh_typo', 'settings' => 'mh_cicero_options[google_webfonts]', 'priority' => 2, 'type' => 'select', 'choices' => array('enable' => __('Enable', 'mh-cicero'), 'disable' => __('Disable', 'mh-cicero'))));
	$wp_customize->add_control('google_webfonts_subsets', array('label' => esc_html__('Google Webfonts Characters', 'mh-cicero'), 'section' => 'mh_typo', 'settings' => 'mh_cicero_options[google_webfonts_subsets]', 'priority' => 3, 'type' => 'select', 'choices' => $mh_google_webfonts_subsets));
	$wp_customize->add_control('font_heading', array('label' => esc_html__('Google Webfont for Headings', 'mh-cicero'), 'section' => 'mh_typo', 'settings' => 'mh_cicero_options[font_heading]', 'priority' => 4, 'type' => 'select', 'choices' => $mh_google_webfonts_list));
	$wp_customize->add_control('font_body', array('label' => esc_html__('Google Webfont for Body Text', 'mh-cicero'), 'section' => 'mh_typo', 'settings' => 'mh_cicero_options[font_body]', 'priority' => 5, 'type' => 'select', 'choices' => $mh_google_webfonts_list));
	$wp_customize->add_control('font_styles', array('label' => esc_html__('Imported Google Font Styles', 'mh-cicero'), 'section' => 'mh_typo', 'settings' => 'mh_cicero_options[font_styles]', 'priority' => 6, 'type' => 'text'));

}
add_action('customize_register', 'mh_cicero_typography_options');
...
if (!function_exists('mh_cicero_custom_fonts')) {
	function mh_cicero_custom_fonts() {
		$custom_fonts = wp_parse_args(
			get_option('mh_cicero_options', array()),
			mh_cicero_default_fonts()
		);
		return $custom_fonts;
	}
}

if (!function_exists('mh_cicero_default_fonts')) {
	function mh_cicero_default_fonts() {
		$default_fonts = array(
			'font_size' => 16,
...
if (!function_exists('mh_cicero_google_webfonts')) {
	function mh_cicero_google_webfonts() {
		$mh_cicero_fonts = mh_cicero_custom_fonts();
		if ($mh_cicero_fonts['google_webfonts'] == 'enable') {
			global $mh_google_webfonts;
			$font_header_text = '';
			$font_heading = '';
			$font_styles = '';
			$font_subset = '';
			if (!empty($mh_cicero_fonts['font_styles'])) {
				$font_styles = ':' . $mh_cicero_fonts['font_styles'];
			}
			if ($mh_cicero_fonts['font_heading'] != $mh_cicero_fonts['font_body']) {
				$font_heading = '%7c' . $mh_google_webfonts[$mh_cicero_fonts['font_heading']]['location'] . esc_attr($font_styles);
			}
			if ($mh_cicero_fonts['google_webfonts_subsets'] == 'latin_ext') {
				$font_subset = '&amp;subset=latin,latin-ext';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'arabic') {
				$font_subset = '&amp;subset=latin,arabic';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'cyrillic') {
				$font_subset = '&amp;subset=latin,cyrillic';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'cyrillic_ext') {
				$font_subset = '&amp;subset=latin,cyrillic,cyrillic-ext';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'greek') {
				$font_subset = '&amp;subset=latin,greek';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'greek_ext') {
				$font_subset = '&amp;subset=latin,greek,greek-ext';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'hebrew') {
				$font_subset = '&amp;subset=latin,hebrew';
			} elseif ($mh_cicero_fonts['google_webfonts_subsets'] == 'vietnamese') {
				$font_subset = '&amp;subset=latin,vietnamese';
			}
			wp_enqueue_style('mh-google-fonts', 'https://fonts.googleapis.com/css?family=' . $mh_google_webfonts[$mh_cicero_fonts['font_body']]['location'] . esc_attr($font_styles) . $font_heading . $font_header_text . $font_subset, array(), null);
		}
	}
}
add_action('wp_enqueue_scripts', 'mh_cicero_google_webfonts');

/***** Include Typography Custom CSS *****/

if (!function_exists('mh_cicero_fonts_css')) {
	function mh_cicero_fonts_css() {
		$mh_cicero_fonts = mh_cicero_custom_fonts();
		if ($mh_cicero_fonts['google_webfonts'] == 'enable') {
			global $mh_google_webfonts;
			if (!empty($mh_cicero_fonts['font_size']) && $mh_cicero_fonts['font_size'] != 16 || $mh_cicero_fonts['font_heading'] != 'open_sans' || $mh_cicero_fonts['font_body'] != 'open_sans') {
				echo '<style type="text/css">' . "\n";
					if (!empty($mh_cicero_fonts['font_size']) && $mh_cicero_fonts['font_size'] != 16) {
						echo '.entry-content { font-size: ' . absint($mh_cicero_fonts['font_size']) . 'px; font-size: ' . absint($mh_cicero_fonts['font_size']) / 16 . 'rem; }' . "\n";
					}
					if ($mh_cicero_fonts['font_heading'] != 'open_sans') {
						echo 'h1, h2, h3, h4, h5, h6, .logo-name { font-family: ' . $mh_google_webfonts[$mh_cicero_fonts['font_heading']]['css'] .'; }' . "\n";
					}
					if ($mh_cicero_fonts['font_body'] != 'open_sans') {
						echo 'body { font-family: ' . $mh_google_webfonts[$mh_cicero_fonts['font_body']]['css'] . '; }' . "\n";
					}
				echo '</style>' . "\n";
			}
		}
...
add_action('wp_head', 'mh_cicero_fonts_css');
