...
if (have_comments()) {
	if (!empty($comments_by_type['comment'])) {
		...
		<h4 class="section-title comments-title"><?php printf(_n('1 Comment on %2$s', '%1$s Comments on %2$s', $comment_count, 'mhc'), number_format_i18n($comment_count), get_the_title()); ?></h4>
	...
	if (get_comments_number() > get_option('comments_per_page')) { ?>
		<div class="comments-pagination">
			<?php paginate_comments_links(array('prev_text' => __('&laquo;', 'mhc'), 'next_text' => __('&raquo;', 'mhc'))); ?>
		</div><?php
	}
	if (!empty($comments_by_type['pings'])) {
		$pings = $comments_by_type['pings'];
		$ping_count = count($comments_by_type['pings']); ?>
		<h4 class="section-title"><?php printf(__('%s Trackbacks & Pingbacks', 'mhc'), $ping_count); ?></h4>
	...
	if (!comments_open()) { ?>
		<p class="no-comments round-corners"><?php _e('Comments are closed.', 'mhc'); ?></p><?php
	}
}
if (comments_open()) {
	$custom_args = array(
    	'title_reply' => __('Leave a comment', 'mhc'),
        'comment_notes_before' => '<p class="comment-notes">' . __('Your email address will not be published.', 'mhc') . '</p>',
        'comment_notes_after'  => '',
