	...
	function mh_breadcrumb() {
		if (!is_home() && !is_front_page()) {
			...
			if ($options['breadcrumbs'] == 'enable') {
				...
				$close_span = '</span>';
				echo '<nav class="breadcrumb">' . $before_link . '<a href="' . esc_url(home_url()) . '" itemprop="url">' . $before_title . __('Home', 'mhc') . $close_span . '</a>' . $close_span . $delimiter;
				if (is_single() && get_post_type() == 'post' && !is_attachment()) {
					...
					echo __('Authors', 'mhc') . $delimiter . esc_attr($user_info->display_name) . '</nav>' . "\n";
				} elseif (is_404()) {
					echo __('Page not found (404)', 'mhc') . '</nav>' . "\n";
				} elseif (is_search()) {
					echo __('Search', 'mhc') . $delimiter . get_search_query() . '</nav>' . "\n";
				} elseif (is_date()) {
					...
					if (is_day()) {
						echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mhc') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter;
						echo $before_link . '<a href="' . $url_month . '" title="' . __('Monthly Archives', 'mhc') . '" itemprop="url">' . $before_title . $arc_month . $close_span . '</a>' . $close_span . $delimiter . $arc_day . ' (' . $arc_day_full . ')';
						echo '</nav>' . "\n";
					} elseif (is_month()) {
						echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mhc') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $arc_month . '</nav>' . "\n";
					} elseif (is_year()) {
						echo $arc_year . '</nav>' . "\n";
					}
