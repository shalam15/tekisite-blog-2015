...
$mh_google_webfonts_subsets = array(
	'arabic' => esc_html__('Arabic', 'mhc'),
	'cyrillic' => esc_html__('Cyrillic', 'mhc'),
	'cyrillic_ext' => esc_html__('Cyrillic Extended', 'mhc'),
	'greek' => esc_html__('Greek', 'mhc'),
	'greek_ext' => esc_html__('Greek Extended', 'mhc'),
	'hebrew' => esc_html__('Hebrew', 'mhc'),
	'latin' => esc_html__('Latin', 'mhc'),
	'latin_ext' => esc_html__('Latin Extended', 'mhc'),
	'vietnamese' => esc_html__('Vietnamese', 'mhc')
);
...
function mh_corporate_typography_options($wp_customize) {
	global $mh_google_webfonts_subsets, $mh_google_webfonts_list;

	/***** Add Section *****/

	$wp_customize->add_section('mh_typo', array('title' => esc_html__('Typography Options', 'mhc'), 'priority' => 3, 'panel' => 'mh_theme_options'));

    /***** Add Settings *****/

    $wp_customize->add_setting('mhc_options[font_size]', array('default' => 14, 'type' => 'option', 'sanitize_callback' => 'mh_corporate_sanitize_integer'));
	$wp_customize->add_setting('mhc_options[google_webfonts]', array('default' => 'enable', 'type' => 'option', 'sanitize_callback' => 'mh_corporate_sanitize_select'));
	$wp_customize->add_setting('mhc_options[google_webfonts_subsets]', array('default' => 'latin', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts_subsets'));
	$wp_customize->add_setting('mhc_options[font_heading]', array('default' => 'open_sans', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts'));
	$wp_customize->add_setting('mhc_options[font_body]', array('default' => 'open_sans', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts'));
	$wp_customize->add_setting('mhc_options[font_styles]', array('default' => '300,400,400italic,600,700', 'type' => 'option', 'sanitize_callback' => 'mh_corporate_sanitize_text'));

    /***** Add Controls *****/

    $wp_customize->add_control('font_size', array('label' => esc_html__('Change default Font Size (px)', 'mhc'), 'section' => 'mh_typo', 'settings' => 'mhc_options[font_size]', 'priority' => 1, 'type' => 'text'));
    $wp_customize->add_control('google_webfonts', array('label' => esc_html__('Google Webfonts', 'mhc'), 'section' => 'mh_typo', 'settings' => 'mhc_options[google_webfonts]', 'priority' => 2, 'type' => 'select', 'choices' => array('enable' => __('Enable', 'mhc'), 'disable' => __('Disable', 'mhc'))));
	$wp_customize->add_control('google_webfonts_subsets', array('label' => esc_html__('Google Webfonts Characters', 'mhc'), 'section' => 'mh_typo', 'settings' => 'mhc_options[google_webfonts_subsets]', 'priority' => 3, 'type' => 'select', 'choices' => $mh_google_webfonts_subsets));
	$wp_customize->add_control('font_heading', array('label' => esc_html__('Google Webfont for Headings', 'mhc'), 'section' => 'mh_typo', 'settings' => 'mhc_options[font_heading]', 'priority' => 4, 'type' => 'select', 'choices' => $mh_google_webfonts_list));
	$wp_customize->add_control('font_body', array('label' => esc_html__('Google Webfont for Body Text', 'mhc'), 'section' => 'mh_typo', 'settings' => 'mhc_options[font_body]', 'priority' => 5, 'type' => 'select', 'choices' => $mh_google_webfonts_list));
	$wp_customize->add_control('font_styles', array('label' => esc_html__('Imported Google Font Styles', 'mhc'), 'section' => 'mh_typo', 'settings' => 'mhc_options[font_styles]', 'priority' => 6, 'type' => 'text'));

}
add_action('customize_register', 'mh_corporate_typography_options');
...
if (!function_exists('mh_corporate_custom_fonts')) {
	function mh_corporate_custom_fonts() {
		$custom_fonts = wp_parse_args(
			get_option('mhc_options', array()),
			mh_corporate_default_fonts()
		);
		return $custom_fonts;
	}
}

if (!function_exists('mh_corporate_default_fonts')) {
	function mh_corporate_default_fonts() {
		$default_fonts = array(
			'font_size' => 14,
...
if (!function_exists('mh_corporate_google_webfonts')) {
	function mh_corporate_google_webfonts() {
		$mh_corporate_fonts = mh_corporate_custom_fonts();
		if ($mh_corporate_fonts['google_webfonts'] == 'enable') {
			global $mh_google_webfonts;
			$font_header_text = '';
			$font_heading = '';
			$font_styles = '';
			$font_subset = '';
			if (!empty($mh_corporate_fonts['font_styles'])) {
				$font_styles = ':' . $mh_corporate_fonts['font_styles'];
			}
			if ($mh_corporate_fonts['font_heading'] != $mh_corporate_fonts['font_body']) {
				$font_heading = '%7c' . $mh_google_webfonts[$mh_corporate_fonts['font_heading']]['location'] . esc_attr($font_styles);
			}
			if ($mh_corporate_fonts['google_webfonts_subsets'] == 'latin_ext') {
				$font_subset = '&amp;subset=latin,latin-ext';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'arabic') {
				$font_subset = '&amp;subset=latin,arabic';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'cyrillic') {
				$font_subset = '&amp;subset=latin,cyrillic';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'cyrillic_ext') {
				$font_subset = '&amp;subset=latin,cyrillic,cyrillic-ext';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'greek') {
				$font_subset = '&amp;subset=latin,greek';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'greek_ext') {
				$font_subset = '&amp;subset=latin,greek,greek-ext';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'hebrew') {
				$font_subset = '&amp;subset=latin,hebrew';
			} elseif ($mh_corporate_fonts['google_webfonts_subsets'] == 'vietnamese') {
				$font_subset = '&amp;subset=latin,vietnamese';
			}
			wp_enqueue_style('mh-google-fonts', 'https://fonts.googleapis.com/css?family=' . $mh_google_webfonts[$mh_corporate_fonts['font_body']]['location'] . esc_attr($font_styles) . $font_heading . $font_header_text . $font_subset, array(), null);
		}
	}
}
add_action('wp_enqueue_scripts', 'mh_corporate_google_webfonts');

/***** Include Typography Custom CSS *****/

if (!function_exists('mh_corporate_fonts_css')) {
	function mh_corporate_fonts_css() {
		$mh_corporate_fonts = mh_corporate_custom_fonts();
		if ($mh_corporate_fonts['google_webfonts'] == 'enable') {
			global $mh_google_webfonts;
			if (!empty($mh_corporate_fonts['font_size']) && $mh_corporate_fonts['font_size'] != 14 || $mh_corporate_fonts['font_heading'] != 'open_sans' || $mh_corporate_fonts['font_body'] != 'open_sans') {
				echo '<style type="text/css">' . "\n";
					if (!empty($mh_corporate_fonts['font_size']) && $mh_corporate_fonts['font_size'] != 14) {
						echo '.entry { font-size: ' . absint($mh_corporate_fonts['font_size']) . 'px; font-size: ' . absint($mh_corporate_fonts['font_size']) / 16 . 'rem; }' . "\n";
					}
					if ($mh_corporate_fonts['font_heading'] != 'open_sans') {
						echo 'h1, h2, h3, h4, h5, h6 { font-family: ' . $mh_google_webfonts[$mh_corporate_fonts['font_heading']]['css'] .'; }' . "\n";
					}
					if ($mh_corporate_fonts['font_body'] != 'open_sans') {
						echo 'body { font-family: ' . $mh_google_webfonts[$mh_corporate_fonts['font_body']]['css'] . '; }' . "\n";
					}
				echo '</style>' . "\n";
			}
		}
...
add_action('wp_head', 'mh_corporate_fonts_css');
