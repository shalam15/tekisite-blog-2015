...
class mh_authors_widget extends WP_Widget {
    function __construct() {
		parent::__construct(
			'mh_authors', esc_html_x('MH Authors', 'widget name', 'mhc'),
			array(
				'classname' => 'mh_authors',
				'description' => esc_html__('MH Authors widget to display a list of authors including the number of published posts.', 'mhc'),
					...
					foreach ($authors as $author) {
						...
						<li class="uw-wrap clearfix">
							<?php if ($instance['avatar_size'] != 'no_avatar') { ?>
								<div class="uw-avatar">
									<a href="<?php echo esc_url(get_author_posts_url($author_ID)); ?>" title="<?php printf(esc_html__('Articles by %s', 'mhc'), $author->display_name); ?>">
										<?php echo get_avatar($author_ID, absint($instance['avatar_size'])); ?>
									</a>
								</div>
							<?php } ?>
							<div class="uw-text">
								<a href="<?php echo esc_url(get_author_posts_url($author_ID)); ?>" title="<?php printf(esc_html__('Articles by %s', 'mhc'), $author->display_name); ?>" class="author-name">
									<?php echo esc_attr($author->display_name); ?>
								</a>
								<p class="uw-data">
									<?php printf(esc_html_x('published %d articles', 'author post count', 'mhc'), count_user_posts($author_ID)); ?>
								</p>
							</div>
						</li><?php
					}
				} else {
					esc_html_e('No authors found', 'mhc');
				} ?>
			</ul><?php
    ...
    function form($instance) {
	    $defaults = array('title' => esc_html__('Authors', 'mhc'), 'authorcount' => 5, 'offset' => 0, 'role' => 0, 'orderby' => 'post_count', 'order' => 'DESC', 'avatar_size' => 48);
        $instance = wp_parse_args($instance, $defaults); ?>
		<p>
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('authorcount')); ?>"><?php esc_html_e('Limit Author Number:', 'mhc'); ?></label>
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('offset')); ?>"><?php esc_html_e('Skip Authors (Offset):', 'mhc'); ?></label>
		...
		<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('role')); ?>"><?php esc_html_e('User Role:', 'mhc'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('role')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('role')); ?>">
				<option value="0" <?php selected(0, $instance['role']); ?>><?php esc_html_e('All Users', 'mhc'); ?></option>
				<option value="administrator" <?php selected('administrator', $instance['role']); ?>><?php esc_html_e('Administrator', 'mhc'); ?></option>
				<option value="editor" <?php selected('editor', $instance['role']); ?>><?php esc_html_e('Editor', 'mhc'); ?></option>
				<option value="author" <?php selected('author', $instance['role']); ?>><?php esc_html_e('Author', 'mhc'); ?></option>
				<option value="contributor" <?php selected('contributor', $instance['role']); ?>><?php esc_html_e('Contributor', 'mhc'); ?></option>
				<option value="subscriber" <?php selected('subscriber', $instance['role']); ?>><?php esc_html_e('Subscriber', 'mhc'); ?></option>
			</select>
        </p>
		<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('orderby')); ?>"><?php esc_html_e('Order Authors by:', 'mhc'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('orderby')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('orderby')); ?>">
				<option value="post_count" <?php selected('post_count', $instance['orderby']); ?>><?php esc_html_e('Number of Posts', 'mhc'); ?></option>
				<option value="display_name" <?php selected('display_name', $instance['orderby']); ?>><?php esc_html_e('User Name', 'mhc'); ?></option>
			</select>
        </p>
		<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('order')); ?>"><?php esc_html_e('Order:', 'mhc'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('order')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('order')); ?>">
				<option value="ASC" <?php selected('ASC', $instance['order']); ?>><?php esc_html_e('Ascending', 'mhc'); ?></option>
				<option value="DESC" <?php selected('DESC', $instance['order']); ?>><?php esc_html_e('Descending', 'mhc'); ?></option>
			</select>
        </p>
		<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('avatar_size')); ?>"><?php esc_html_e('Avatar Size in px:', 'mhc'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('avatar_size')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('avatar_size')); ?>">
				...
				<option value="no_avatar" <?php selected('no_avatar', $instance['avatar_size']); ?>><?php esc_html_e('No Avatars', 'mhc'); ?></option>
			</select>
        </p><?php
    }
}
