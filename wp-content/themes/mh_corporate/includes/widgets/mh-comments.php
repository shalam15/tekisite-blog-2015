...
class mh_comments_widget extends WP_Widget {
    function __construct() {
		parent::__construct(
			'mh_comments', esc_html_x('MH Recent Comments', 'widget name', 'mhc'),
			array(
				'classname' => 'mh_comments',
				'description' => esc_html__('MH Recent Comments widget to display your recent comments including user avatars.', 'mhc'),
						...
						<li class="uw-wrap clearfix">
							...
							<div class="uw-text">
								<span class="mh-recent-comments-author">
									<?php printf(esc_html_x('%1$s on %2$s', 'comment widget', 'mhc'), $comment->comment_author, ''); ?>
								</span>
    ...
    function form($instance) {
	    $defaults = array('title' => esc_html__('Recent Comments', 'mhc'), 'number' => 5, 'offset' => 0, 'avatar_size' => 48);
		$instance = wp_parse_args($instance, $defaults); ?>
		<p>
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Limit Comment Number:', 'mhc'); ?></label>
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('offset')); ?>"><?php esc_html_e('Skip Comments (Offset):', 'mhc'); ?></label>
