...
class mh_joystick_facebook_page extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_joystick_facebook_page', esc_html_x('MH Facebook Page', 'widget name', 'mh-joystick'),
			array(
				'classname' => 'mh_joystick_facebook_page',
				'description' => esc_html__('Widget to display your Facebook page on your website.', 'mh-joystick')
			)
		);
	}
    function widget($args, $instance) {
    	$defaults = array('title' => esc_html__('Follow on Facebook', 'mh-joystick'), 'fb_url' => 'https://www.facebook.com/MHthemes', 'width' => 350, 'height' => 500, 'cover' => 0, 'faces' => 1, 'posts' => 0);
        ...
        echo $args['before_widget'];
			...
			if (!empty($instance['fb_url'])) {
	    		echo '<div class="fb-widget"><div class="fb-page" data-href="' . esc_url($instance['fb_url']) . '" data-width="' . absint($instance['width']) . '" data-height="' . absint($instance['height']) . '" data-hide-cover="' . esc_attr($instance['cover']) . '" data-show-facepile="' . esc_attr($instance['faces']) . '" data-show-posts="' . esc_attr($instance['posts']) . '"></div></div>'. "\n";
			}
    ...
    function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('fb_url')); ?>"><?php esc_html_e('Facebook Page URL:', 'mh-joystick'); ?></label>
       	...
       	<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php esc_html_e('Width (min. 280px):', 'mh-joystick'); ?></label>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('height')); ?>"><?php esc_html_e('Height (min. 130px):', 'mh-joystick'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('cover')); ?>"><?php esc_html_e('Hide cover photo', 'mh-joystick'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('faces')); ?>"><?php esc_html_e('Show profile photos', 'mh-joystick'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('posts')); ?>"><?php esc_html_e('Show posts from FB page', 'mh-joystick'); ?></label>
		</p><?php
    }
}
