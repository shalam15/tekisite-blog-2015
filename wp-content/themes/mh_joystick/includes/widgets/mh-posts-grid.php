...
class mh_joystick_posts_grid extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_joystick_posts_grid', esc_html_x('MH Posts Grid', 'widget name', 'mh-joystick'),
			array(
				'classname' => 'mh_joystick_posts_grid',
				'description' => esc_html__('Display a grid of posts including thumbnail images.', 'mh-joystick'),
        ...
        echo $args['before_widget'];
			if ($widget_posts->have_posts()) :
				...
				while ($widget_posts->have_posts()) : $widget_posts->the_post();
					...
					if ($counter >= 1) { ?>
						<article <?php post_class('mh-col-1-3 cp-grid-item clearfix'); ?>>
