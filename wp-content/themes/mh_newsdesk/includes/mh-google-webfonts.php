...
$mh_google_webfonts_subsets = array(
	'arabic' => esc_html__('Arabic', 'mh-newsdesk'),
	'cyrillic' => esc_html__('Cyrillic', 'mh-newsdesk'),
	'cyrillic_ext' => esc_html__('Cyrillic Extended', 'mh-newsdesk'),
	'greek' => esc_html__('Greek', 'mh-newsdesk'),
	'greek_ext' => esc_html__('Greek Extended', 'mh-newsdesk'),
	'hebrew' => esc_html__('Hebrew', 'mh-newsdesk'),
	'latin' => esc_html__('Latin', 'mh-newsdesk'),
	'latin_ext' => esc_html__('Latin Extended', 'mh-newsdesk'),
	'vietnamese' => esc_html__('Vietnamese', 'mh-newsdesk')
);
...
function mh_newsdesk_typography_options($wp_customize) {
	global $mh_google_webfonts_subsets, $mh_google_webfonts_list;

	/***** Add Section *****/

	$wp_customize->add_section('mh_typo', array('title' => esc_html__('Typography', 'mh-newsdesk'), 'priority' => 3, 'panel' => 'mh_theme_options'));
	...
	$wp_customize->add_setting('mh_newsdesk_options[google_webfonts_subsets]', array('default' => 'latin', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts_subsets'));
	$wp_customize->add_setting('mh_newsdesk_options[font_heading]', array('default' => 'oswald', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts'));
	$wp_customize->add_setting('mh_newsdesk_options[font_body]', array('default' => 'pt_serif', 'type' => 'option', 'sanitize_callback' => 'mh_sanitize_google_webfonts'));
    ...
    $wp_customize->add_control('google_webfonts', array('label' => esc_html__('Google Webfonts', 'mh-newsdesk'), 'section' => 'mh_typo', 'settings' => 'mh_newsdesk_options[google_webfonts]', 'priority' => 2, 'type' => 'select', 'choices' => array('enable' => esc_html__('Enable', 'mh-newsdesk'), 'disable' => esc_html__('Disable', 'mh-newsdesk'))));
	$wp_customize->add_control('google_webfonts_subsets', array('label' => esc_html__('Google Webfonts Characters', 'mh-newsdesk'), 'section' => 'mh_typo', 'settings' => 'mh_newsdesk_options[google_webfonts_subsets]', 'priority' => 3, 'type' => 'select', 'choices' => $mh_google_webfonts_subsets));
	$wp_customize->add_control('font_heading', array('label' => esc_html__('Google Webfont for Headings', 'mh-newsdesk'), 'section' => 'mh_typo', 'settings' => 'mh_newsdesk_options[font_heading]', 'priority' => 4, 'type' => 'select', 'choices' => $mh_google_webfonts_list));
	$wp_customize->add_control('font_body', array('label' => esc_html__('Google Webfont for Body Text', 'mh-newsdesk'), 'section' => 'mh_typo', 'settings' => 'mh_newsdesk_options[font_body]', 'priority' => 5, 'type' => 'select', 'choices' => $mh_google_webfonts_list));
...
add_action('customize_register', 'mh_newsdesk_typography_options');
...
if (!function_exists('mh_newsdesk_custom_fonts')) {
	function mh_newsdesk_custom_fonts() {
		$custom_fonts = wp_parse_args(
			get_option('mh_newsdesk_options', array()),
			mh_newsdesk_default_fonts()
		);
		return $custom_fonts;
	}
}

if (!function_exists('mh_newsdesk_default_fonts')) {
	function mh_newsdesk_default_fonts() {
		$default_fonts = array(
			'font_size' => 17,
...
if (!function_exists('mh_newsdesk_google_webfonts')) {
	function mh_newsdesk_google_webfonts() {
		$mh_newsdesk_fonts = mh_newsdesk_custom_fonts();
		if ($mh_newsdesk_fonts['google_webfonts'] == 'enable') {
			global $mh_google_webfonts;
			$font_header_text = '';
			$font_heading = '';
			$font_styles = '';
			$font_subset = '';
			if (!empty($mh_newsdesk_fonts['font_styles'])) {
				$font_styles = ':' . $mh_newsdesk_fonts['font_styles'];
			}
			if ($mh_newsdesk_fonts['font_heading'] != $mh_newsdesk_fonts['font_body']) {
				$font_heading = '%7c' . $mh_google_webfonts[$mh_newsdesk_fonts['font_heading']]['location'] . esc_attr($font_styles);
			}
			if ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'latin_ext') {
				$font_subset = '&amp;subset=latin,latin-ext';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'arabic') {
				$font_subset = '&amp;subset=latin,arabic';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'cyrillic') {
				$font_subset = '&amp;subset=latin,cyrillic';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'cyrillic_ext') {
				$font_subset = '&amp;subset=latin,cyrillic,cyrillic-ext';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'greek') {
				$font_subset = '&amp;subset=latin,greek';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'greek_ext') {
				$font_subset = '&amp;subset=latin,greek,greek-ext';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'hebrew') {
				$font_subset = '&amp;subset=latin,hebrew';
			} elseif ($mh_newsdesk_fonts['google_webfonts_subsets'] == 'vietnamese') {
				$font_subset = '&amp;subset=latin,vietnamese';
			}
			wp_enqueue_style('mh-google-fonts', 'https://fonts.googleapis.com/css?family=' . $mh_google_webfonts[$mh_newsdesk_fonts['font_body']]['location'] . esc_attr($font_styles) . $font_heading . $font_header_text . $font_subset, array(), null);
		}
	}
}
...
if (!function_exists('mh_newsdesk_fonts_css')) {
	function mh_newsdesk_fonts_css() {
		...
		if ($mh_newsdesk_fonts['google_webfonts'] == 'enable') {
			global $mh_google_webfonts;
			if (!empty($mh_newsdesk_fonts['font_size']) && $mh_newsdesk_fonts['font_size'] != 17 || $mh_newsdesk_fonts['font_heading'] != 'oswald' || $mh_newsdesk_fonts['font_body'] != 'pt_serif') {
				echo '<style type="text/css">' . "\n";
					if (!empty($mh_newsdesk_fonts['font_size']) && $mh_newsdesk_fonts['font_size'] != 17) {
						echo '.entry-content p { font-size: ' . absint($mh_newsdesk_fonts['font_size']) . 'px; font-size: ' . absint($mh_newsdesk_fonts['font_size']) / 16 . 'rem; }' . "\n";
					}
					if ($mh_newsdesk_fonts['font_heading'] != 'oswald') {
						echo '.header-nav, .header-sub, h1, h2, h3, h4, h5, h6, .breadcrumb, .footer-bottom, .widget-list li, .cat-item, .page_item, .menu-item, .button, .submit, .commentlist .vcard, .pagination, .mh-share-buttons { font-family: ' . $mh_google_webfonts[$mh_newsdesk_fonts['font_heading']]['css'] .'; }' . "\n";
					}
					if ($mh_newsdesk_fonts['font_body'] != 'pt_serif') {
						echo 'body { font-family: ' . $mh_google_webfonts[$mh_newsdesk_fonts['font_body']]['css'] . '; }' . "\n";
					}
				echo '</style>' . "\n";
			}
		}
