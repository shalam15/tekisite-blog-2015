...
if (!function_exists('mh_purity_breadcrumb')) {
	function mh_purity_breadcrumb() {
		if (!is_home() && !is_front_page()) {
			global $post;
			$mh_purity_options = mh_purity_theme_options();
			if (isset($mh_purity_options['breadcrumbs']) && $mh_purity_options['breadcrumbs']) {
				...
				$close_span = '</span>';
				echo '<nav class="breadcrumb">' . $before_link . '<a href="' . esc_url(home_url()) . '" itemprop="url">' . $before_title . __('Home', 'mhp') . $close_span . '</a>' . $close_span . $delimiter;
				if (is_single() && get_post_type() == 'post' && !is_attachment()) {
					...
					echo get_the_title() . '</nav>' . "\n";
				} elseif (is_attachment()) {
					echo __('Media', 'mhp') . $delimiter;
					echo get_the_title() . '</nav>' . "\n";
					...
					echo single_cat_title('', false) . '</nav>' . "\n";
				} elseif (is_tag()) {
					echo single_term_title('', false) . '</nav>' . "\n";
				} elseif (is_author()) {
					global $author;
					$user_info = get_userdata($author);
					echo __('Authors', 'mhp') . $delimiter . esc_attr($user_info->display_name) . '</nav>' . "\n";
				} elseif (is_404()) {
					echo __('Page not found (404)', 'mhp') . '</nav>' . "\n";
				} elseif (is_search()) {
					echo __('Search', 'mhp') . $delimiter . get_search_query() . '</nav>' . "\n";
				} elseif (is_date()) {
					...
					if (is_day()) {
						echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mhp') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter;
						echo $before_link . '<a href="' . $url_month . '" title="' . __('Monthly Archives', 'mhp') . '" itemprop="url">' . $before_title . $arc_month . $close_span . '</a>' . $close_span . $delimiter . $arc_day . ' (' . $arc_day_full . ')';
						echo '</nav>' . "\n";
					} elseif (is_month()) {
						echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mhp') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $arc_month . '</nav>' . "\n";
					} elseif (is_year()) {
						echo $arc_year . '</nav>' . "\n";
					}
...
add_action('mh_before_post_content', 'mh_purity_breadcrumb');
add_action('mh_before_page_content', 'mh_purity_breadcrumb');
