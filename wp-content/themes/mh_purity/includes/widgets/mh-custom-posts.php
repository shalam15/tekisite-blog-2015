...
class mh_purity_custom_posts_widget extends WP_Widget {
    function __construct() {
		parent::__construct(
			'mh_custom_posts', esc_html_x('MH Custom Posts', 'widget name', 'mhp'),
			array(
				'classname' => 'mh_custom_posts',
				'description' => esc_html__('Custom Posts Widget to display posts based on categories or tags.', 'mhp'),
				'customize_selective_refresh' => true
			)
		);
	}
    function widget($args, $instance) {
	    $defaults = array('title' => '', 'link' => '', 'category' => 0, 'cats' => '', 'tags' => '', 'postcount' => 5, 'offset' => 0, 'order' => 'date', 'excerpt_length' => 110, 'meta' => 0, 'sticky' => 1);
		...
		echo $args['before_widget'];
			...
			<div class="cp-widget clearfix"><?php
				while ($widget_loop->have_posts()) : $widget_loop->the_post();
					if ($counter == 1) { ?>
						<div class="cp-widget-item cp-large cp-count-<?php echo absint($instance['postcount']); ?>">
