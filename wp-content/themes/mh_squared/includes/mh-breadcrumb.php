...
if (!function_exists('mh_squared_breadcrumbs')) {
	function mh_squared_breadcrumbs() {
		if (!is_home() && !is_front_page()) {
			global $post;
			$mh_squared_options = mh_squared_theme_options();
			if ($mh_squared_options['breadcrumbs'] == 'enable') {
				$delimiter = '';
				...
				$close_span = '</span>';
				echo '<nav class="breadcrumb">' . $before_link . '<a href="' . esc_url(home_url()) . '" itemprop="url" class="bc-home">' . $before_title . __('Home', 'mh-squared') . $close_span . '</a>' . $close_span . $delimiter;
					if (is_single() && get_post_type() == 'post' && !is_attachment()) {
						...
						echo $before_text . __('Media', 'mh-squared') . $close_span;
						...
						echo $before_text . __('Author Archive', 'mh-squared') . $close_span;
					} elseif (is_404()) {
						echo $before_text . __('Page not found (404)', 'mh-squared') . $close_span;
					} elseif (is_search()) {
						echo $before_text . __('Search', 'mh-squared') . $close_span;
					} elseif (is_date()) {
						...
						if (is_day()) {
							echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mh-squared') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter;
							echo $before_link . '<a href="' . $url_month . '" title="' . __('Monthly Archives', 'mh-squared') . '" itemprop="url">' . $before_title . $arc_month . $close_span . '</a>' . $close_span . $delimiter . $before_text . $arc_day . $close_span;
						} elseif (is_month()) {
							echo $before_link . '<a href="' . $url_year . '" title="' . __('Yearly Archives', 'mh-squared') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $before_text . $arc_month . $close_span;
						} elseif (is_year()) {
...
add_action('mh_squared_before_post_content', 'mh_squared_breadcrumbs');
add_action('mh_squared_before_page_content', 'mh_squared_breadcrumbs');
