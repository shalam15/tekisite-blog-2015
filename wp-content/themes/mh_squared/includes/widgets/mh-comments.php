...
class mh_squared_comments extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_squared_comments', esc_html_x('MH Recent Comments', 'widget name', 'mh-squared'),
			array(
				'classname' => 'mh_squared_comments',
				'description' => esc_html__('MH Recent Comments widget to display your recent comments including user avatars.', 'mh-squared'),
				...
				if ($comments) {
					foreach ($comments as $comment) { ?>
						<li class="rc-item clearfix">
							...
							<div class="rc-text">
								<div class="rc-author">
									<?php printf(_x('%1$s on %2$s', 'comment widget', 'mh-squared'), $comment->comment_author, ''); ?>
								</div>
    ...
    function form($instance) {
        $defaults = array('title' => esc_html__('Recent Comments', 'mh-squared'), 'number' => 5, 'offset' => 0);
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Limit Comment Number:', 'mh-squared'); ?></label>
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('offset')); ?>"><?php esc_html_e('Skip Comments (Offset):', 'mh-squared'); ?></label>
