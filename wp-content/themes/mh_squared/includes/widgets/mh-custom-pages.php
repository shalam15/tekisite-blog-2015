...
class mh_squared_custom_pages extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_squared_custom_pages', esc_html_x('MH Custom Pages', 'widget name', 'mh-squared'),
			array(
				'classname' => 'mh_squared_custom_pages',
				'description' => esc_html__('Display any of your pages with this widget.', 'mh-squared'),
    ...
    function update($new_instance, $old_instance) {
		...
		if (!empty($new_instance['pages'])) {
			$instance['pages'] = mh_squared_sanitize_id_list($new_instance['pages']);
		}
    ...
    function form($instance) {
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('pages')); ?>"><?php esc_html_e('Filter Pages by ID (comma separated):', 'mh-squared'); ?></label>
		...
		<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('excerpt')); ?>"><?php esc_html_e('Display Excerpt:', 'mh-squared'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('excerpt')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('excerpt')); ?>">
				<option value="enable" <?php selected('enable', $instance['excerpt']); ?>><?php esc_html_e('Enable', 'mh-squared') ?></option>
				<option value="disable" <?php selected('disable', $instance['excerpt']); ?>><?php esc_html_e('Disable', 'mh-squared') ?></option>
			</select>
        </p><?php
    }
}
