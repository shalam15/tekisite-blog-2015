...
class mh_squared_posts_large extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_squared_posts_large', esc_html_x('MH Posts Large', 'widget name', 'mh-squared'),
			array(
				'classname' => 'mh_squared_posts_large',
				'description' => esc_html__('Display posts (large-sized) including thumbnail images.', 'mh-squared'),
				'customize_selective_refresh' => true
			)
		);
	}
	function widget($args, $instance) {
		$defaults = array('title' => '', 'link' => '', 'category' => 0, 'cats' => '', 'tags' => '', 'postcount' => 1, 'offset' => 0, 'order' => 'date', 'sticky' => 1);
        ...
        echo $args['before_widget'];
			if ($widget_posts->have_posts()) :
				...
				echo '<div class="cp-large-widget">' . "\n";
					while ($widget_posts->have_posts()) : $widget_posts->the_post(); ?>
						<article <?php post_class('cp-large-item'); ?>>
							...
							<div class="cp-large-date">
								<?php echo get_the_date(); ?>
							</div>
							<div class="cp-large-excerpt">
								<?php the_excerpt(); ?>
							</div>
						</article><?php
					endwhile;
